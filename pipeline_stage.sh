...
mkdir -p "$ROOT"
python "$SCRIPT_DIR/train.py" --packet_mode stage --model gauss --stages "$STAGES" --bits "$BITS" --batch "${BATCH:-36}" \
  --epochs "${EPOCHS:-200}" --warmup_epochs "${WARMUP_EPOCHS:-20}" --snr_min "${SNR_MIN:-0}" --snr_max "${SNR_MAX:-10}" \
  --norm --out_dir "$ROOT/train" --imagenet_root "$IMAGENET_ROOT" "${FADING_ARGS[@]}"

python "$SCRIPT_DIR/estimate_information.py" --ckpt "$ROOT/train/best.pt" --packet_mode stage --model gauss --stages "$STAGES" --bits "$BITS" \
  --batch "${INFO_BATCH:-8}" --epochs "${INFO_EPOCHS:-10}" --norm --out "$ROOT/information.json" --imagenet_root "$IMAGENET_ROOT"
